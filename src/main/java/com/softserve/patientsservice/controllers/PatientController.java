...
import com.softserve.patientsservice.handler.PatientStreamBridge;
...
public class PatientController {
    ...
    private final PatientStreamBridge patientStreamBridge;
    ...
    public ResponseEntity<String> deactivatePatientByMPI(@PathVariable String patientMPI) {
        if (patientService.deactivatePatientByMPI(patientMPI) == 1) {
            patientStreamBridge.send("deactivatePatientByMPI-out-0", patientMPI);
